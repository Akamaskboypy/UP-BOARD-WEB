...
export default function SubjectCard({ subj, data, progress, onToggle, subjName }: SubjectCardProps) {
  ...
  const branchColors = {
    ...
    inorganic: 'bg-[#fef3c7] text-[#a16207]',
    organic: 'bg-[#dcfce7] text-[#15803d]',
  };

  const titleText = data.title || (data.chapters.length > 1 ? data.unit?.replace(/^Unit \d+ — /, '') : data.chapters[0]);

  return (
    <div className="bg-white rounded-xl shadow-[0_3px_16px_rgba(0,0,0,0.09)] mb-3.5 overflow-hidden">
      {data.sec && (
        <div className="px-3.5 py-1.5 text-[9px] font-bold tracking-widest uppercase bg-[#f8f8f8] border-b border-[#f0f0f0]">
          {data.sec}
        </div>
      )}
